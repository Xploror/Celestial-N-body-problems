function [x0_corr, t_corr, phi] = diffcorr_l1(x0, mu, tStep, delx)
...
while abs(x(3))>1*10^(-6) || count<2
    x(3)
    % preparing init conditions for next iteration by updating with STM matrix...
    t = 0; % time-count
    del_xt = [0;0-x(2);0-x(3);0];
    del_x0_req = inv(phi)*del_xt;
    if x(1)<(1-mu) && x(1)>(-mu)
        x0_corr = x0_corr + [0;0;0;0.0001*del_x0_req(4)];
        %x0_corr(4) = 1.00005*x0_corr(4);
    else
        x0_corr(4) = 0.95*x0_corr(4);
    end
    x = x0_corr;
    X = [x0];
    phi = eye(4); % State Transition Matrix (Monodromy matrix)
    while sign(x(2))*sign(x_prev(2))==1 || t<100*tStep  % tolerance (position crossing x-axis perpendicularly) & dont stop before 1 iterations of tStep
        ...
        omega = (1-mu)/(sqrt((x(1)+mu)^2 + x(2)^2))^3 + mu/(sqrt((x(1)-(1-mu))^2 + x(2)^2))^3;
        a = 1+2*omega;
        b = 1-omega;
        A = [0 0 1 0
             0 0 0 1
...
%     plot(X(1,:),X(2,:), '*r', 1-mu, 0, 'og')
%      %hold on;
%     yline([0])
%     pause(0.1)
   
    if abs(x3_prev-x(3))<10^(-8) && x(3)<10^(-4)
        k = k + 1;
    else
        x3_prev = x(3);
        k = 0;
