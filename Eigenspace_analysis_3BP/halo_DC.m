function [x0_corr, t_corr, phi] = halo_DC(x0, mu, tStep)

% Jacobian of dynamics x_dot = f(x) (3D xyz plane) for L1
t = 0; % time-count
phi = eye(length(x0));
x0_corr = x0;
x = x0_corr;
...
while min(abs(x(4)),abs(x(6)))>1*10^(-5) || count<10
    min(abs(x(4)),abs(x(6)))
    % preparing init conditions for next iteration by updating with STM matrix...
    t = 0; % time-count
    del_xt = [0;0;0;0-x(4);0;0-x(6)]; % desired velx nd velz == 0
    del_x0_req = phi^(-1)*del_xt;
    if x(1)<(1-mu) && x(1)>(-mu)
        x0_corr = x0_corr - [0.001*del_x0_req(1);0;0;0;0.0001*del_x0_req(5);0];
        %x0_corr(5) = 0.1*x0_corr(5);
    else
        x0_corr(5) = 0.99*abs(x0_corr(5));
    end
    x = x0_corr;
    X = [x0];
...
%     pause(0.1)
    ...
    if (abs(x4_prev-x(4))<10^(-6) && x(4)<10^(-3)) %&& (abs(x6_prev-x(6))<10^(-6) && x(6)<10^(-3))
        k = k + 1;
    else
        x4_prev = x(4);
        x6_prev = x(6);
