function [x0_corr, t_corr, phi] = diffcorr_l1_FBP(x0, mu, mu1, p, n, tStep, delx)
...
while abs(x(3))>5*10^(-7) || count<2
    ...
    if x(1)<(1-mu) && x(1)>(-mu)
        x0_corr = x0_corr + [0;0;0;0.0001*del_x0_req(4);0];
    ...
    x = x0_corr;
    X = [x0];
    phi = eye(5); % State Transition Matrix (Monodromy matrix)
...
%     pause(0.1)
    ...
    if abs(x3_prev-x(3))<10^(-8) && x(3)<10^(-3)
        k = k + 1;
    else
        x3_prev = x(3);
        k = 0;
